...
contract("RhodeIT Deployment", (err) => {
    ...
    it('should down vote a url', async () => {
        var receipt = await AmangaContract.methods.up_down_vote_url(url, 0,accounts[3]).send({
            gas: 8000000,
            from: accounts[0]
        })
    ...
    it('should down vote a given article 2 times', async () => {
        for (var i = 0; i < 2; i++) {
            var reported = await AmangaContract.methods.up_down_vote_url(url, 0,accounts[4]).send({
                gas: 8000000,
                from: accounts[3]
            })
        }
    ...
    it('should up vote a given article 2 times', async () => {
        for (var i = 0; i < 2; i++) {
            var receipt = await AmangaContract.methods.up_down_vote_url(url, 1,accounts[5]).send({
                gas: 8000000,
                from: accounts[0]
            })
        }
